...
- `lib/projects-server.ts` espone `getProjects()` (server-only): in **dev** fa fetch di `${NEXT_PUBLIC_API}/projects` (in dev = `.env.local` → API locale su localhost:3001) e restituisce `pyld` se non vuoto, con fallback allo snapshot su errore/array vuoto; in **build/produzione** restituisce `projectsData` senza rete (build deterministica). È usato da `generateStaticParams()`, `generateMetadata()` e dal componente in `app/progetti/[slug]/page.tsx` e dall'`ItemList` JSON-LD in `app/layout.tsx`: in dev un progetto presente solo nell'API locale ha subito la sua pagina dettaglio (la griglia già faceva refresh dall'API locale via client). Nota: cambiare i dati dell'API locale mentre `next dev` è attivo potrebbe richiedere un restart del dev server per ricalcolare `generateStaticParams`.
- Le pagine `/progetti/[slug]/` sono server component generate da `generateStaticParams()` (v. `getProjects()` sopra; in dev da API locale, a build dallo snapshot): la card in `project-card.tsx` è un `<a>` reale verso la pagina (niente modal — eliminato). Ogni pagina progetto include Nav e Footer e ha una freccia `&larr;` accanto al titolo che torna alla homepage (niente breadcrumb visibile; il JSON-LD `BreadcrumbList` resta per SEO). La sitemap e `llms.txt`/`llms-full.txt` includono le pagine progetto.
