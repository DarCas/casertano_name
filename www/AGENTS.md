...
- `npm run build` — build statica in `out/` (prebuild: `extract-version.mjs` + `fetch-projects.mjs` + `generate-llms.mjs`; postbuild: `generate-sitemap.mjs` + `inline-css.mjs` + `stamp-sw.mjs`)
...
- `NEXT_PUBLIC_CMS_API` — base URL Strapi (include `/api`), usata da `scripts/lib/strapi.mjs` (`fetchProjects`) e da `lib/projects.ts` (`fetchProjects`: `getProjects()` server-side + refresh client). `.env` = CMS pubblico, `.env.local` = localhost. I media Strapi hanno URL relativi (`/uploads/...`): la mappatura li rende assoluti col origin del CMS
