...
app.post("/api/contact", contactLimiter, async (req, res) => {
    if (!toEmail) {
        res.status(500).json({ error: "Contatti non configurati" })
        return
    }
    ...
    if (!valid) {
        res.status(400).json({ error: "Captcha non valido" })
        return
    }
    ...
    try {
        ...
        res.status(500).json({ error: "Errore nell'invio dell'e-mail. Riprova più tardi." })
    }
})
