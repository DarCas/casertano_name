...
Express also serves the built frontend from `../www` (immutable, `maxAge: 1y`); non-`/api/*` misses serve `www/404.html`. ETag is disabled globally (`app.disable('etag')`) — static re-enables it via `etag: true`.
