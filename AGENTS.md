...
`NEXT_PUBLIC_NAME` sets the name shown in the footer; `NEXT_PUBLIC_VAT` shows fiscal info below it.
`NEXT_PUBLIC_SOCIAL_GITHUB`, `NEXT_PUBLIC_SOCIAL_LINKEDIN`, `NEXT_PUBLIC_SOCIAL_TELEGRAM` each show a social icon in the footer when set.
...
- **`lib/`** — `projects.ts` (types + `fetchProjects()`), `skills.ts` (9 `SkillCategory` groups → skills section), `utils.ts` (`hashDelay` only), `version.ts` (auto-generated by `predev`/`prebuild` from `package.json`).
...
- **`scripts/generate-sitemap.mjs`** — postbuild script, generates `out/sitemap.xml`. `scripts/inline-css.mjs` — postbuild script, inlines critical CSS. `scripts/extract-version.mjs` — predev/prebuild script, writes `package.json` version into `lib/version.ts`.
...
- License: CC BY-NC-ND 4.0 (`LICENSE.md`). Node v22 (`.nvmrc`).
