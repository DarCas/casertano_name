...
- **`components/`** — 10 files: Nav, Hero, Projects, ProjectCard, ProjectModal, Skills, Contact, EmailLink, Footer, NetworkBg.
- **`content/projects.json`** — project data. Typed via `Project` in `lib/projects.ts` with `features: string[]`.
- **`lib/`** — `projects.ts` (types) + `utils.ts`.
- **`api/`** — separate Express app (own `package.json` + `tsconfig.json`). `POST /api/contact` validates via Zod, sends via Nodemailer. Runs via `tsx`, no build. Excluded from root `tsconfig.json` `exclude`.
...
- `components.json` exists (shadcn/ui config) but **no** shadcn components are installed.
